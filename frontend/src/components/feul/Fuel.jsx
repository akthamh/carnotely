...
export default function Fuels() {
    ...
    const fetchFuels = async () => {
        ...
        try {
            ...
            toast.error("Failed to fetch fuel logs. Please try again.");
        } finally {
    ...
    const handleDelete = async (fuelId) => {
        ...
        if (result.isConfirmed) {
            try {
                ...
                MySwal.fire("Deleted!", "The fuel log has been removed.", "success");
            } catch (error) {
    ...
    const handleFormSubmit = async (fuelData) => {
        try {
            setFormErrors({});
            if (editingFuel) {
                const response = await axiosInstance.patch(`/fuels/${editingFuel._id}`, fuelData);
                setFuels((prev) =>
                    prev.map((fuel) => (fuel._id === editingFuel._id ? response.data : fuel))
                );
